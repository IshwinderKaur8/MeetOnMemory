  ...
  useEffect(() => {
    const fetchLineage = async () => {
      try {
        ...
        if (res.data?.success) setLineage(res.data.lineage);
      } catch (err) {
          ...
          {lineage.map((d) => (
            <div key={d._id} className="border-l-2 border-blue-500 pl-4 py-2">
              <p className="text-sm text-gray-500">
                {new Date(d.createdAt).toLocaleDateString()} — {d.sourceMeetingId?.title}
              </p>
              <p className="font-medium">{d.text}</p>
              <span className="text-xs uppercase text-gray-400">{d.status}</span>
            </div>
          ))}
        </div>
      </div>
